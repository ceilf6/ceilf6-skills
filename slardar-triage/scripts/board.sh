...
state=""; issue=""; title=""; mr_id=""; sid_opt=""
while [ $# -gt 0 ]; do
  case "$1" in
    --state) state=$2; shift 2;; --issue-id) issue=$2; shift 2;; --title) title=$2; shift 2;;
    --mr-id) mr_id=$2; shift 2;; --session-id) sid_opt=$2; shift 2;;
...
printf '%s' "$existing" | jq --arg b "$cur_branch" --arg n "Meego ${meego_url} · Task ${task_id}" \
  --arg mi "$meego_id" --arg mu "$meego_url" --arg mr "$mr_id" '
  . + {branch:$b, status:(.status // "active"), note:(.note // $n), milestones:(.milestones // {})}
  + (if $mi == "" then {} else {meego_id:$mi, meego_type:"issue", meego_url:$mu} end)
  + (if $mr == "" then {} else {mr_id:$mr} end)' > "$ctx/meta.json.tmp" && mv "$ctx/meta.json.tmp" "$ctx/meta.json"
