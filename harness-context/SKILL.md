...
description: 按 git 分支管理当前需求的本地上下文仓（<仓库根>/.harness-ceilf6/<分支>/，本地为真源、wiki 为沉淀）。动作：init 初始化并从需求 wiki 子文档导入种子；add 随时存入飞书 IM 群聊消息区间、飞书文档、Meego、MR 评论或自由文本；get 取出全部上下文；为组合式沉淀（如配合 /lark-sediment）供料并记台账。当用户要求初始化需求上下文、导入种子、「把这段消息/这个文档/这个 MR 的评论存进上下文」、取出/装载需求上下文、或沉淀需求经验时使用。init/add/get 完成后默认自动接续 harness-ceilf6 开始开发（用户明确「只存不跑」时豁免）。定位是入口与仓管：抓取由当前 agent 用 lark-cli / bytedcli 完成。
---
