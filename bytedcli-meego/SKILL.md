...
{ "repos": { "lark/byteview-web": {
    ...
    "story": {
      "done_transition": ["需求提出", "技术评审", "需求开发", "需求测试", "需求合入"],
      "schedule_node": "需求开发",
      "dev_roles": ["tech_owner"],
      "create_fields": [
      ...
      "node_forms": { "技术评审": [
        ...
        { "field_key": "field_d40cc0", "field_value": "option_2" } ] } },
...
仓库已绑定空间但配置缺映射项（`template_id` / `schedule_node` / `dev_owner_key` / `done_transition` / `node_forms` / `dev_roles` / `done_state` / `create_fields`，缺哪项由用到它的子命令或服务端报出）时：
1. 读一个真实条目：`bytedcli --json meego node get --project-key <pk> --work-item-id <id>`（story 节点流，`list[]` 的顺序不是流转顺序，顺序要按页面节点图定）与 `bytedcli --json meego state list …`（issue 状态流）；
2. 按「事实完成才流转」原则提出建议映射：done_transition 列本端从起点到终点要经过的全部节点，别人负责的节点与自动节点（如「已上车」）不进映射；带必填表单的节点把字段与固定取值写进 node_forms；
...
- **confirm 应答成功不等于节点真的完成（2026-08-19 真机）**：见过 `node transition` 退 0、应答 `status:success`，而节点回读仍是 `doing`。所以 advance 每次 confirm 后回读校验节点是否 `finished`，不然看板收到的是「已流转完成」的假报告。判断真假只能靠回读，别信应答。回读也有传播延迟，同日见过紧接的 `node get` 还是 `doing`、隔一拍就 `finished`，所以首读不 `finished` 时 advance 再读一次才判死——只读一次会把已生效的流转误报成未生效。
...
advance 停下的原因：被别人的节点挡住、某个必填表单项 node_forms 没覆盖（服务端报 `{field} 必填`）、表单值被拒、confirm 回读未生效。角色空缺与节点负责人空缺已由 advance 自愈，不在此列。下面是人工处理步骤，全部以本人身份、只动本人 owner 的节点 / 字段。
