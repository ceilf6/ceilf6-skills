...
4. 核验测试真实性：diff 中的测试必须真实断言验收行为——只执行不断言、断言恒真、或与验收标准无关的测试是 finding（通常 major）。红绿过程证据在仓库 `.harness-ceilf6/<分支名，/ 替换为 __>/tdd-evidence.md`：文件存在时读取核验「红灯确实红过、豁免理由是否成立」；变更含可断言行为而证据缺失时按 minor 记录。
