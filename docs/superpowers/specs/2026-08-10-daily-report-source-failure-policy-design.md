...
- Lark 用户身份、基础 `bytedcli` 身份、Bits、Meego 普通 OAuth 继续沿用
  当前硬依赖策略。
- 无人值守任务发现需要用户配置的条件时，通过飞书私聊当前用户提醒；不发群
  消息、不发邮件。
...
runner 是唯一通知执行者，代理不直接发消息。通知目标固定为 profile 中的
`feishu_open_id`，仅允许两类消息：
...
6. 同一通知指纹重复执行只发送一次；不同日期可以再次发送。
7. 私聊发送失败不覆盖日报主退出码。
8. 安装器从 Git 真源安装后，runner、prompt、测试和技能文件与运行副本一致。
9. 全量现有 runner、本地 AI 解析器和发布脚本测试继续通过。
