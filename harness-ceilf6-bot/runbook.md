...
- **口径**：harness 线程台账（`ht list` 的默认视图：未归档且 `status != done`）∪ `/tasks` 列出的运行时在册任务，
  两本账按分支去重。台账是「等人工 CR」那批任务唯一的落脚处——bot 建完 MR 就走终态、把任务从自己的
  在册表里销账，只数 `/tasks` 会把最该限流的那批全数漏掉。运行时那一路补的是台账还看不见的：排队中、
  启动中，以及台账已归档、bot 这边仍可 `/resume` 的滞留任务。**办事条目除外**——它不产 MR、不等人工 CR，
  不该让几件杂活把群里的接单能力挂住。限的是人处理不过来，不是机器跑不过来——后者由 `concurrency` 管，
  两个数各管各的。
- **判定要读台账**：每条新任务消息在入队前跑一次 `threads.sh list --json`（20 个线程量级约 3s），接单表情
  因此比以前晚几秒。判定串行排队，连发几条不会各自读到同一份入队前的账而双双放行。
- **台账读不出来就降级**：脚本不在、缺 `jq`、超时（30s）等，退回只数运行时在册任务，并在
  `logs/launchd.err.log` 留一行「本次按运行时在册数计」。限流不整个失效，但口径退回等人工 CR 的不算数。
...
- 拒单在 `logs/launchd.err.log` 里有一行 `拒单 <message_id>（未完成 N ≥ 上限）：<正文首行>`。@ 旁路怀疑失效时看这一行的
  正文形态，按 mention 的实际字面改 `botName`。
- 开机在 `logs/launchd.err.log` 报一行 `接单水位 N/上限`：台账路径不通平时要等到第一条该拒的消息才现形，
  而那时的现象是「该拒的没拒」，没人会去查。
...
6. **接单水位**：把 `maxOpenTasks` 临时改成 1 并重装 → 群里发一条新任务，话题里收到「先不接新单」回复，且不出现 👀、`state/queue.jsonl` 里没有它；再发一条 `@harness-ceilf6 …` 的，照常 👀 起跑。验完改回 5。回执里报的数应与 `ht list` 的行数对得上，差额只该来自排队中/启动中/已归档的滞留任务。
