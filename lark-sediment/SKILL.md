---
name: lark-sediment
description: 把当前会话里值得长期保留、跨会话/跨人有用的经验（bug 根因与决定性证据、决策与取舍理由、可复用的排查/验证方法、踩坑与环境结论）沉淀进飞书云文档知识库「字节vc」。当用户说「沉淀」「沉淀一下」「沉淀到飞书/云文档」「记录到飞书」「归档到知识库」「同步到 lark 文档」，或一段排查/决策收尾时触发。技能会自动在知识库中定位最合适的位置、先去重再决定追加或新建子文档，自主选定后直接写入、事后汇报（不打断用户等确认）。harness 需求场景下另有 B 线四问叙事语料（发现/解决/反思/总结，转正/述职取材），每次沉淀必产、与 A 线技术沉淀同批写入同一篇需求子文档。不用于本地 memory / 仓库 docs（那是本地层），不用于纯代码提交。
---
...
- **B 线：叙事语料**（转正/述职/面试的产出语料，字段对齐 report-writer-bytedance 的 WorkEvent schema）——completeness 优先：只要存在需求上下文，每次沉淀**必产一条**四问叙事节，不做价值筛选，唯一标准是如实覆盖本轮关键节点；无需求上下文时自然跳过。B 线不走第 3 步定位、不走第 4 步去重（落点即需求子文档）；增量对照 `sediment.md` 台账与文档内已有叙事节，只写上次收尾之后的新节点。
